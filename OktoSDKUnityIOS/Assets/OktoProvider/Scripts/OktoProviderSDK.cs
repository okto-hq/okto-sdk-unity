...
namespace OktoProvider
{
    public class OktoProviderSDK : MonoBehaviour
    {
        ...
        public OktoProviderSDK(string apikey, string buildType, string webClientId)
        {
            /*credentials = Resources.Load<Credentials>("Credentials");
            this.apiKey = credentials.apiKey;*/
            InitializePlayGamesLogin(webClientId);
            this.apiKey = apikey;
        ...
        void InitializePlayGamesLogin(string webClientId)
        {
        ...
        public async Task<(AuthDetails result, Exception error)> LoginGoogle()
        {
            ...
            try
            {
                // Perform sign-in and wait for authentication to complete
        ...
        public async Task<List<string>> readContractData(ContractRequestData data)
        {
            ApiResponse<List<string>> response = await MakePostRequest<List<string>>($"/v1/readContractData", data);
            return response.data;
        }
